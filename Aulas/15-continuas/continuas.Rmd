...
date: 'Criado em 13/11/2020<br />Versão de `r format(Sys.Date(), "%d/%m/%Y")`'
lang: 'pt'
...
-   Um programa para gerar números aleatórios no intervalo $[0, 10]$.
...
-   A probabilidade de um número gerado estar entre $5$ e $7$ é de $$
    P(5 < X < 7) = P(5 \leq X \leq 7) = \int_5^7 f(x)dx = \int_5^7 \frac{1}{10}dx = \frac{1}{5}
    $$

-   O valor esperado de $X$ é $$
    E(X) = \int_{-\infty}^{\infty} xf(x)dx = \int_{0}^{10} \frac{1}{10}xdx = 5
    $$

-   A variância de $X$ é $$
    \sigma^2(X) 
      = \int_{-\infty}^{+ \infty} (x - 5)^2 \cdot f(x) dx 
      = \int_{0}^{10} (x-5)^2 \cdot \frac{1}{10} dx 
      = \frac{25}{3}
      = `r fm(25/3)`
    $$

-   **Exercício:** calcule $\sigma^2(X)$ usando a fórmula
...
* Variância: $$\sigma^2(X) = \frac{(a - b)^2}{12}$$
...
* **Exercício:** os números `min` e `max` podem ser gerados?
...
* Segundo o [Levantamento do Perfil Antropométrico da População Brasileira Usuária do Transporte Aéreo Nacional](https://www2.anac.gov.br/arquivos/pdf/Relatorio_Final_Projeto_Conhecer.pdf), em 2009, a estatura do homem brasileiro adulto era distribuída normalmente, com média $1{,}73$m e desvio padrão $0{,}07$m.
...
* Média: $\mu \in \mathbb{R}$

* Desvio padrão: $\sigma \in \mathbb{R}_{\geq 0}$
...
```{r entre-180-190}
pnorm(1.90, mu_brasil, sigma_brasil) - pnorm(1.80, mu_brasil, sigma_brasil)
```
...
```{r entre-180-190-h}
pnorm(1.90, mu_holanda, sigma_holanda) - pnorm(1.80, mu_holanda, sigma_holanda)
```
...
* Segundo nossa simulação, esta é a probabilidade de um holandês ser mais baixo que um brasileiro.
