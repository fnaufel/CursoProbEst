...
* Se tivermos a equação desta reta, podemos 
  ...
  1. [Prever]{.hl} o peso de pessoas cujas alturas não estavam nos dados originais.

* A equação da reta vai ser da forma
  $\quad$ [$\widehat{\text{peso}}(x) = \widehat{b_0} + \widehat{b_1} \cdot \text{altura}(x)$.]{.hl}

* Os "chapéus" em $\widehat{\text{peso}}$, $\widehat{b_0}$, e em $\widehat{b_1}$ significam que os valores são [estimativas]{.hl}, e não valores exatos.
...
* Qual reta queremos? A que deixe [o mínimo possível de resíduos.]{.hl}

* Mas, em vez de considerar os resíduos, que podem ser negativos ou positivos, vamos considerar os [quadrados dos resíduos]{.hl}, mais ou menos como no cálculo da variância.
...
* Exemplos de retas diferentes, com seus valores de S:
...
* Desenvolvendo o quadrado no final:
...
* Confirme suas respostas usando o R:
  ...
  1. Compute, sobre as variáveis padronizadas
  ...
  1. Construa o *scatterplot* das variáveis padronizadas e use `geom_smooth` (como no exemplo acima) para desenhar a reta de regressão.
...
* Vamos repetir o *scatterplot* com a reta de regressão:

    ```{r echo=FALSE}
    regressao
    ```

* Os resíduos mostram se o modelo é adequado.

  1. Devemos verificar se os resíduos seguem uma distribuição normal.
     ...
     Ou usando um gráfico quantil-quantil:
     ...
     Ou fazendo um teste estatístico de normalidade:
     ...
     Este resultado diz que [não devemos considerar estes resíduos como sendo normais.]{.hl} Os resíduos à direita atrapalham muito. Se ignorarmos todos os resíduos maiores que $10$kg, o teste retorna um resultado mais compatível com a normalidade (mas não muito):
  ...
  2. Devemos verificar se os resíduos não dependem dos valores estimados. 
  
     A idéia é que o *scatterplot* dos resíduos pelos valores estimados [não deve apresentar nenhum padrão]{.hl}. A nuvem de pontos deve se distribuir de forma aparentemente aleatória em torno da reta horizontal $y = 0$ (a média dos resíduos), sem *outliers* e sem alterações significativas da variância.
...
* O pacote `gglm` (http://graysonwhite.com/gglm/) tem `stat`s para estas verificações:
...
* Se você não se importar com a feiúra dos gráficos do R base, pode usar a função `plot`:

    ```{r}
    plot(modelo, c(1, 2))
    ```

* As observações com os maiores resíduos, $3$ delas marcadas com os números nos gráficos, são
...
* A distribuição dos resíduos [não deve se afastar da normal:]{.hl}

    ```{r echo=FALSE}
    set.seed(1234)
    
    tibble(
      resíduos = c(rnorm(80, -2), rnorm(80, 2))
    ) %>% 
      ggplot(aes(resíduos)) +
...
* O *scatterplot* [não deve ter padrões visíveis:]{.hl}
    ...
    n <- 100
    tibble(
      estimados = runif(n, -20, 20),
      resíduos  = (estimados^2) / 50 + rnorm(n, sd = 3)
    ) %>% 
...
* No *scatterplot*, [os resíduos não devem ter variabilidade diferente, dependendo do valor estimado:]{.hl}
    ...
    tibble(
      ...
      resíduos  = estimados + 10 + rnorm(n, sd = estimados + 10)
    ) %>% 
          ggplot(aes(estimados, resíduos)) +
            geom_point() +
        labs(
...
* Como a equação da reta de regressão usa a correlação [linear]{.hl}, devemos verificar que este é o tipo de correlação entre as duas variáveis. Um *scatterplot* é uma boa maneira de visualizar isto.

* Não deve haver [*outliers*]{.hl} extremos.

* Uma vez definida a reta de regressão, devemos [examinar os resíduos]{.hl}. No *scatterplot* de resíduos pelos valores estimados, a nuvem de pontos deve estar espalhada em torno da reta $y = 0$, sem padrões óbvios, sem curvatura, sem alterações na variância à medida que percorremos a reta horizontal..
...
* Os resíduos parecem não ter variância constante.

* Além disso, parece haver uma certa curvatura na nuvem de pontos.
